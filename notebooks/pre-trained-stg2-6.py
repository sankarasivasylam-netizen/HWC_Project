...
class CNN_stage2_dn201(torch.nn.Module):
    def __init__(self, num_classes, input_size=(256, 256), channels=3):
        super(CNN_stage2_dn201, self).__init__()

        self.input_size = input_size
        self.channels = channels

        self.network = densenet201(weights=DenseNet201_Weights.DEFAULT)
...
def model_freeze_unfreeze(model,epoch,model_name):
    ...
    if epoch == 0:
        ...
        if "r34" in model_name:
            ...
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.fc.parameters(), "lr": 3e-4}
                                        ])
        else:
            # Unfreeze final classifier layer
            ...
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.classifier.parameters(), "lr": 3e-4}
                                        ])
    ...
    if epoch == 30:
        ...
        if "r34" in model_name:
            # Unfreeze last layer
            for param in model.network.layer4.parameters():
                param.requires_grad = True    
                
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.layer4.parameters(), "lr": 1e-4},
                                          {"params": model.network.fc.parameters(), "lr": 2e-4}
                                        ])
        elif "mnv2" in model_name:
            # Unfreeze all layers
            for param in model.network.features[-2:].parameters():
                param.requires_grad = True
                
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.features[-2:].parameters(), "lr": 1e-5},
                                          {"params": model.network.classifier.parameters(), "lr": 2e-4}
                                        ])
        elif "dn121" in model_name or "dn201" in model_name:
            # Unfreeze last layer
            for param in model.network.features.denseblock4.parameters():
                param.requires_grad = True
            
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.features.denseblock4.parameters(), "lr": 1e-4},
                                          {"params": model.network.classifier.parameters(), "lr": 2e-4}
                                        ])
    ...
    if epoch == 60:
        ...
        if "r34" in model_name:
            # Unfreeze last layer
            for param in model.network.layer3.parameters():
                param.requires_grad = True    
                
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.layer4.parameters(), "lr": 1e-5},
                                          {"params": model.network.layer3.parameters(), "lr": 1e-5},
                                          {"params": model.network.fc.parameters(), "lr": 1e-4}])
        elif "mnv2" in model_name:
            # Unfreeze all layers
            for param in model.network.features[-4:].parameters():
                param.requires_grad = True     
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.features[-4:].parameters(), "lr": 1e-6},
                                          {"params": model.network.classifier.parameters(), "lr": 1e-4}
                                        ])
        elif "dn121" in model_name or "dn201" in model_name:
            # Unfreeze last layer
            for param in model.network.features.denseblock3.parameters():
                param.requires_grad = True
            
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.features.denseblock4.parameters(), "lr": 1e-5},
                                          {"params": model.network.features.denseblock3.parameters(), "lr": 1e-5},
                                          {"params": model.network.classifier.parameters(), "lr": 1e-4}
                                        ])
        ...
        if "r34" in model_name:
            ...
            optimizer = torch.optim.AdamW([
                                          {"params": model.network.features.parameters(), "lr": 1e-6},
                                          {"params": model.network.classifier.parameters(), "lr": 1e-5}
                                        ])
        elif "dn121" in model_name or "dn201" in model_name:
...
def train_model(model,train_loader_s2,val_loader_s2,device,output_dir):
        ...
        for epoch in range(num_epochs):
            ...
            mix_transform = v2.RandomApply(
                [v2.RandomChoice([mixup, cutmix])],
                p=mix_prob
            )
           
            if epoch in (0,30,60,85):
                optimizer = model_freeze_unfreeze(model,epoch,model_name)
...
def main():
        ...
        output_dir = BASE_DIR / "output" / "Stage2" / "pretrained" / "V6"
        ...
        train_time= train_model(res_model,train_loader_s2,val_loader_s2,device,output_dir)
        ...
        train_time = train_model(mnv2_model,train_loader_s2,val_loader_s2,device,output_dir)
        eval_model(mnv2_model,val_loader_s2,device,output_dir,class_names,train_time)
        ...
        train_time = train_model(dn121_model,train_loader_s2,val_loader_s2,device,output_dir)
        eval_model(dn121_model,val_loader_s2,device,output_dir,class_names,train_time)'''

        print('-----------------------------------')
        print("Densenet201 Model eval and training")
        print('-----------------------------------')
        # Densenet201
        dn201_model = CNN_stage2_dn201(13).to(device)
        train_time = train_model(dn201_model,train_loader_s2,val_loader_s2,device,output_dir)
        eval_model(dn201_model,val_loader_s2,device,output_dir,class_names,train_time)
