...
#### Pretrained DenseNet121,ResNet34,MobileNetV2 Stage 1 - Training and Evaluation script
...
class CNN_stage1_r34(torch.nn.Module):
    ...
    Initial retraining only on classifier. Layer4, Layesr3 unfreezing in Phase 2
...
class CNN_stage1_mnv2(torch.nn.Module):
    ...
    Initial retraining only on classifier. Inverted residual blocks 11-17 + Convolution (block 18)
    retrained from Phase 2.
...
class CNN_stage1_dn121(torch.nn.Module):
    ...
    Initial retraining only on classifier. Denseblock3 + Denseblock4 retrained from 
    Phase 2.
...
def get_optim_scheduler(model,model_name,phase):
    """
    ...
    """
    # LR based on hyperprameter tuning
    # Higher learning rate for classifier and lower for backbone
    if "mnv2" in model_name:
        if phase == 1:
           optimizer = torch.optim.AdamW([
                {'params': list(model.network.classifier.parameters()),     'lr': 1e-4}
            ])
        elif phase == 2:
		
           params_list = list(model.network.features[11:].parameters())
			
           optimizer = torch.optim.AdamW([
                {'params': list(model.network.features[11:].parameters()),   'lr': 3e-5},
                {'params': list(model.network.classifier.parameters()),      'lr': 1e-4}
            ])
        ...
        if phase == 1:
           ...
           params_list = list(model.network.features.denseblock3.parameters()) + \
                         ...
                         list(model.network.features.denseblock4.parameters()) + \
                         list(model.network.features.norm5.parameters())
						  
           optimizer = torch.optim.AdamW([
                {'params': list(model.network.features.denseblock3.parameters()) +
                           list(model.network.features.transition3.parameters()), 'lr': 3e-5},
                {'params': list(model.network.features.denseblock4.parameters()) +
                           list(model.network.features.norm5.parameters()),       'lr': 3e-5},
                {'params': list(model.network.classifier.parameters()),           'lr': 1e-4}
            ])
        ...
        if phase == 1:
           optimizer = torch.optim.AdamW([
                {'params': list(model.network.fc.parameters()),     'lr': 1e-4}
            ])
        elif phase == 2:
           params_list = list(model.network.layer3.parameters()) + \
                         list(model.network.layer4.parameters())
				
           optimizer = torch.optim.AdamW([
                {'params': list(model.network.layer3.parameters()), 'lr': 3e-5},
                {'params': list(model.network.layer4.parameters()), 'lr': 3e-5},
                {'params': list(model.network.fc.parameters()),     'lr': 1e-4}
            ])
...
def train_model(model,train_loader_s1,val_loader_s1,device,output_dir, train_transforms,val_transform,train_transforms_2,val_transform_2,train_ds_stage1,val_ds_stage1):
        ...
        for epoch in range(num_epochs):
            ...
            #Progressive unfreezing by calling get_optim_scheduler function
            if epoch == 0:
			
                optimizer, scheduler = get_optim_scheduler(model,model_name,1)
				
                train_ds_stage1.transforms_dict = train_transforms
                val_ds_stage1.transforms_dict = val_transform
                ...
                optimizer, scheduler = get_optim_scheduler(model,model_name,2)
...
def grad_camplus(file,model,model_name,output_dir,device):
        ...
        for idx, (img_path, true_label) in enumerate(data[0:20]):
		        ...
		        rgb_img = np.array(pil_img.resize((256,256))).astype(np.float32)  / 255.0
...
def main():
        ...
        #Input image files organized into respective stage folders(Organic/Nonorganic) inside
        ...
        output_dir = BASE_DIR / "output" / "Stage1" / "pretrained" /  "V7"
        ...
        updated_metadata_df = pd.concat(
            [organic_df,nonorganic_df,val_df, test_df],# organic_dup
            ignore_index=True
        ).sample(frac=1)
        ...
        class_counts = [non_organic_count, organic_count]  # non-organic, organic
        class_weights = 1.0 / torch.tensor(class_counts, dtype=torch.float)
        #Assign per-sample weight based on class weights
        sample_weights = class_weights[class_labels]  # per-sample weight
        ...
        train_time,best_model_file = train_model(res_model,train_loader_s1,val_loader_s1,device,output_dir,train_transforms,val_transform,
		                                        train_transforms_2,val_transform_2,train_ds_stage1,val_ds_stage1)
		#Final evaluation on test set
        eval_model(res_model,best_model_file,test_loader_s1,device,output_dir,class_names,train_time,val_file,test_file)
        ...
        train_time,best_model_file= train_model(mnv2_model,train_loader_s1,val_loader_s1,device,output_dir,train_transforms,val_transform,
		                                        train_transforms_2,val_transform_2,train_ds_stage1,val_ds_stage1)
        eval_model(mnv2_model,best_model_file,test_loader_s1,device,output_dir,class_names,train_time,val_file,test_file)
        ...
        train_time,best_model_file = train_model(dn121_model,train_loader_s1,val_loader_s1,device,output_dir,train_transforms,val_transform,
		                                        train_transforms_2,val_transform_2,train_ds_stage1,val_ds_stage1)
        eval_model(dn121_model,best_model_file,test_loader_s1,device,output_dir,class_names,train_time,val_file,test_file)
