...
# Ensemble Pre-Trained Version 4 - Stage 2
...
class EnsembleModel(torch.nn.Module):   
    def __init__(self, num_classes,modelA, modelB):
        super().__init__()
        self.modelA = modelA
        self.modelB = modelB
        self.classifier = torch.nn.Linear(num_classes * 2, num_classes)
...
def dynamic_confidence_ensemble_eval(modelA, modelB,val_loader_s2,device,output_dir,class_names):
	...
	with torch.no_grad():
		for images, labels, sources, type_images in val_loader_s2:
			...
			prob_A = F.softmax(outputs_A, dim=1)
			prob_B = F.softmax(outputs_B, dim=1)

			conf_A,_ = torch.max(prob_A, dim=1)
			conf_B,_ = torch.max(prob_B, dim=1)
			...
			ensemble_prob = w_A * prob_A + w_B * prob_B
	...
	model_name = 'ensemble_mvn2_dn121'
...
def logit_ensemble_eval(modelA, modelB,val_loader_s2,device,output_dir,class_names,w_A,w_B,only_acc):
	...
	with torch.no_grad():
		for images, labels, sources, type_images in val_loader_s2:
			...
			final_output= w_A * outputs_A + w_B * outputs_B
...
def main():
        ...
        output_dir = BASE_DIR / "output" / "Stage2" / "Ensemble" / "V5"
        pth_dir = BASE_DIR / "output" / "Stage2" / "pretrained" / "V3"


        r34_files = [
            os.path.join(pth_dir, f)
            for f in os.listdir(pth_dir)
            if "r34" in f and f.endswith(".pth")
        ]

        r34_pth = max(r34_files, key=os.path.getmtime)
        ...
        dn121_files = [
            os.path.join(pth_dir, f)
            for f in os.listdir(pth_dir)
            if "dn121" in f and f.endswith(".pth")
        ]

        dn121_pth = max(dn121_files, key=os.path.getmtime)
        ...
        mnv2_ckpt = torch.load(pth_dir/mnv2_pth, map_location=device,weights_only=True)
        mnv2_model.load_state_dict(mnv2_ckpt["model_state_dict"])
        
        dn121_model = CNN_stage2_dn121(13).to(device)
        dn121_ckpt = torch.load(pth_dir/dn121_pth, map_location=device,weights_only=True)
        dn121_model.load_state_dict(dn121_ckpt["model_state_dict"])
        ...
        #print('------------------------------------')
        #print("Dynamic weightedEnsemble Model eval ")
        #print('------------------------------------')
        
        #dynamic_confidence_ensemble_eval(mnv2_model,dn121_model,val_loader_s2,device,output_dir,class_names)

        print('------------------------------------')
        print("Logit weightedEnsemble Model eval ")
        print('------------------------------------')
        ...
        logit_ensemble_eval(mnv2_model,dn121_model,val_loader_s2,device,output_dir,class_names,best_weight,1-best_weight,only_acc='N')
