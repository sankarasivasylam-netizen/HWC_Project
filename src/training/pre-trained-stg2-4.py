...
class CNN_stage2_cnextt(torch.nn.Module):
    def __init__(self, num_classes, input_size=(224, 224), channels=3):
        super(CNN_stage2_cnextt, self).__init__()

        self.input_size = input_size
        self.channels = channels

        self.network = convnext_tiny(weights=ConvNeXt_Tiny_Weights.IMAGENET1K_V1.DEFAULT)

        num_features = self.network.classifier[2].in_features
        self.network.classifier =   torch.nn.Sequential(
                                    torch.nn.Flatten(),
                                    torch.nn.Linear(num_features, 13)
                                )
    ...
    def freeze(self):
        ...
        # Unfreeze 2nd last layer
        for param in self.network.features[5].parameters():
            param.requires_grad = True

        # Unfreeze last layer
        for param in self.network.features[7].parameters():
            param.requires_grad = True
...
def main():
        ...
        print('--------------------------------')
        cnext_model = CNN_stage2_cnextt(13).to(device)
        train_time = train_model(cnext_model,train_loader_s2,val_loader_s2,device,output_dir)
        eval_model(cnext_model,val_loader_s2,device,output_dir,class_names,train_time)
