...
def unfreeze_last_layers(module, n_last_layers=1):
    layers = list(module.children())
    for layer in layers[-n_last_layers:]:
        for p in layer.parameters():
            p.requires_grad = True

def unfreeze_densenet_last_block(densenet):
    for p in densenet.features.denseblock4.parameters():
        p.requires_grad = True            
...
def main():
        ...
        output_dir = BASE_DIR / "output" / "Stage2" / "pretrained" / "V4"
        ...
        print(r34_pth)
        res_ckpt = torch.load(pth_dir/r34_pth, map_location=device,weights_only=True)
        res_model.load_state_dict(res_ckpt["model_state_dict"])
