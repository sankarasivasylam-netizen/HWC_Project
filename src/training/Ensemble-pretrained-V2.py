...
# Ensemble Pre-Trained Version 2 - Stage 2
...
class CNN_stage2_mnv2(torch.nn.Module):
    def __init__(self, num_classes, input_size=(256, 256), channels=3):
        ...
        self.network.classifier =  torch.nn.Linear(num_features, num_classes)
...
def main():
        ...
        output_dir = BASE_DIR / "output" / "Stage2" / "Ensemble" / "V2"
        pth_dir = BASE_DIR / "output" / "Stage2" / "pretrained" / "V6" # replaced V3 with V6
        ...
        mnv2_files = [
            os.path.join(pth_dir, f)
            for f in os.listdir(pth_dir)
            if "best_CNN_stage2_mnv2" in f and f.endswith(".pth")
        ]
        ...
        dn121_files = [
            os.path.join(pth_dir, f)
            for f in os.listdir(pth_dir)
            if "best_CNN_stage2_dn121" in f and f.endswith(".pth")
        ]
        ...
        print(pth_dir/mnv2_pth)
        print(pth_dir/dn121_pth)
        ...
        dn121_ckpt = torch.load(pth_dir/dn121_pth, map_location=device)# removed weights_only = TRUE
        ...
        # Resnet34
        print('--------------------------------')
        print("Ensemble Model eval and training")
        print('--------------------------------')
        cnn_ensemble_model = EnsembleModel(13,mnv2_model,dn121_model).to(device)

        # freeze all
        for p in cnn_ensemble_model.parameters():
            p.requires_grad = False

        # unfreeze classifier
        for p in cnn_ensemble_model.classifier.parameters():
            p.requires_grad = True

        # unfreeze MobileNetV2 last block
        for p in cnn_ensemble_model.modelA.network.features[-1].parameters():
            p.requires_grad = True

        # unfreeze DenseNet121 last block
        for p in cnn_ensemble_model.modelB.network.features.denseblock4.parameters():
            p.requires_grad = True

        for name, p in cnn_ensemble_model.named_parameters():
            if p.requires_grad:
                print("Trainable:", name)
        
        train_time = train_model(cnn_ensemble_model,train_loader_s2,val_loader_s2,device,output_dir)
        eval_model(cnn_ensemble_model,val_loader_s2,device,output_dir,class_names,train_time)
